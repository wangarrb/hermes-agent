...
_CLAUDE_IDLE_MARKERS = (
    "❯",  # Claude Code v2.1+ prompt (Unicode U+276F)
    ">",  # Claude Code prompt prefix (ASCII fallback)
    "claude>",  # Alternative prompt prefix
    "? for shortcuts",  # Help hint in status bar
    "bypass permissions",  # Status bar hint
    "/help",  # Command hint
)
...
class ClaudeInteractiveListener(BaseInteractiveListener):
    ...
    # ── Override: on_post_inject — Claude Code queues long input ──
    def on_post_inject(
        self, args: argparse.Namespace, *,
        zellij_session: str, zellij_pane_id: str, log_path: Path,
    ) -> None:
        """Claude Code v2.1 queues multi-word input and requires a second Enter to submit.
