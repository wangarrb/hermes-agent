...
class StreamDeliveryMixin:
    ...
    def _reset_stream_delivery_tracking(self) -> None:
        ...
        if think_scrubber is not None or toolcall_scrubber is not None:
            ...
            if tail:
                deliver(ctx_scrubber.feed(tail) if ctx_scrubber is not None else tail)
    ...
    def _fire_stream_delta(self, text: str) -> None:
        ...
        if isinstance(text, str):
            ...
            text = toolcall_scrubber.feed(text) if toolcall_scrubber is not None else text
    ...
    def _scrub_tool_call_stream_text(self, text: str) -> str:
        """Drop text-channel tool-call XML while leaving reasoning tags intact."""
        if not text:
            return text
        scrubber = getattr(self, "_stream_toolcall_scrubber", None)
        return scrubber.feed(text) if scrubber is not None else text

    def _forward_suppressed_stream_text(self, text: str) -> None:
        """Forward tool-call-suppressed text after removing leaked XML markup."""
