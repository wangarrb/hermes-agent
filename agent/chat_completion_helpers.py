...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def _route_suppressed_text(self, text: str) -> None:
        ...
        if self.agent.stream_delta_callback:
            visible = self.agent._scrub_tool_call_stream_text(text)
            if visible:
                self._quiet(lambda: (self.agent.stream_delta_callback(visible), self.agent._record_streamed_assistant_text(visible)))
