...
class ContextCompressor(ContextEngine):
    ...
    def _compute_threshold_tokens(
        ...
        ``max_tokens`` is a configured output ceiling, not a reservation made on
        every request.  It must therefore not shrink the compaction threshold:
        custom providers commonly configure a large ceiling (for example 128K
        on a 200K context window), while their ordinary turns have far smaller
        outputs.  Treating that ceiling as permanently reserved would compact a
        200K session at roughly 64K tokens.
        """
