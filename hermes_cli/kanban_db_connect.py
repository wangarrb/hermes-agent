    ...
    with _cross_process_init_lock(path):
        ...
        def _init_if_needed(conn: sqlite3.Connection) -> None:
            ...
            if resolved not in _INITIALIZED_PATHS:
                ...
                # The fork's interactive Kanban listeners use an additive
                # generation/rework/control/delivery contract that upstream
                # v0.21.4 does not ship in its kernel. Keep the upstream base
                # schema authoritative, then apply the fork extension tables
                # and columns in the same init lock.
                ensure_fork = getattr(_kb, "ensure_fork_compat_schema", None)
                if callable(ensure_fork):
                    ensure_fork(conn)
