...
class CLISessionMixin:
    ...
    def _handle_mycompress_command(self, cmd_original: str = ""):
        """Run the skill-guided compression wrapper through the shared core."""
        raw_args = (cmd_original or "").strip().split(None, 1)
        raw_args = raw_args[1] if len(raw_args) > 1 else ""
        try:
            from agent.mycompress import load_mycompress_focus, parse_mycompress_args
            keep_last, user_focus = parse_mycompress_args(raw_args)
            focus_topic = load_mycompress_focus(
                user_focus, task_id=getattr(self, "session_id", None), runtime_note="cli"
            )
        except (LookupError, ValueError) as exc:
            print(f"(._.) /mycompress failed: {exc}")
            return
        self._manual_compress(
            "/compress", keep_last_override=keep_last, focus_topic_override=focus_topic
        )
