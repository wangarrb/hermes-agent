...
class Task:
    ...
    def common_dir(self) -> Optional[str]:
        ...
        return str(contract["common_dir"]) if contract and contract.get("common_dir") else None
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        return cls(
            ...
            **{col: g(col) or None for col in _TASK_EMPTY_IS_NULL_COLUMNS},
            ...
            generation=int(g("generation", 1) or 1),
            rework_hold=bool(g("rework_hold", 0)),
            rework_baseline_fingerprint=g("rework_baseline_fingerprint"),
        )
...
_TASK_OPTIONAL_COLUMNS = (
    "branch_name", "base_commit", "target_branch", "workspace_contract_json", "delivery_state", "delivery_json",
    "project_id", "tenant", "result", "idempotency_key", "worker_pid",
            ...
            with write_txn(conn, allow_nested=True):
                ...
                if result_subscriber:
                    conn.execute(
                        "INSERT OR IGNORE INTO kanban_result_subscriptions "
                        "(task_id, target_profile, created_at, active) VALUES (?, ?, ?, 1)",
                        (task_id, _canonical_assignee(result_subscriber), now),
                    )
            return task_id
...
def add_comment(
    ...
    *,
    ...
    if expected_run_id is not None or expected_generation is not None or expected_claim_lock is not None:
        row = conn.execute(
            "SELECT current_run_id, generation, claim_lock FROM tasks WHERE id = ?", (task_id,)
        ).fetchone()
        if row is None:
            return 0
...
def _append_event(
    ...
    # Fork listeners consume a second, profile-scoped result queue. Keep the
    # upstream event log authoritative, then mirror actionable events into the
    # additive queue when that extension schema is present.
    try:
        compat = _fork_compat_module()
        compat._enqueue_result_notifications(
            conn, task_id=task_id, event_id=int(cur.lastrowid), kind=kind,
            payload=payload, now=now,
        )
    except Exception:
        logger.debug("fork result-notification mirror skipped", exc_info=True)
...
def reclaim_task(
    ...
    *,
    ...
    row = conn.execute(
        "SELECT status, claim_lock, worker_pid, worker_started_at, current_run_id, generation "
        "FROM tasks WHERE id = ?", (task_id,),
    ).fetchone()
    if not row:
        return False
    ...
    *,
    ...
    with write_txn(conn):
        cur_row = conn.execute(
            "SELECT status, block_kind, block_recurrences, generation, claim_lock "
            "FROM tasks WHERE id = ?", (task_id,),
        ).fetchone()
        if cur_row is None:
            return False
        if expected_generation is not None and int(cur_row["generation"] or 1) != int(expected_generation):
            return False
        if expected_claim_lock is not None and (cur_row["claim_lock"] or "") != expected_claim_lock:
            return False
...
# The local fork has a durable Kanban delivery/control contract that is not yet
# present in the v0.21.4 upstream kernel. Keep it behind a lazy compatibility
# module so the upstream facade and its split siblings remain authoritative for
# upstream features, while the fork's existing listeners can retain their
# generation/rework/reservation/result-notification behaviour. The extension is
# intentionally loaded only when one of those fork symbols is requested.
_FORK_COMPAT_MODULE = "hermes_cli.kanban_fork_compat"


def _fork_compat_module():
    import importlib
    return importlib.import_module(_FORK_COMPAT_MODULE)


def ensure_fork_compat_schema(conn: sqlite3.Connection) -> None:
    """Apply the fork-only additive Kanban schema/migrations to an open DB.

    Upstream v0.21.4 owns the base schema and connection lifecycle. The fork
    extension only adds nullable/defaulted columns and fork-owned tables; its
    migration is idempotent and runs under the upstream connection init lock.
    """
    compat = _fork_compat_module()
    conn.executescript(compat.SCHEMA_SQL)
    compat._migrate_add_optional_columns(conn)


def _fork_compat_attr(name: str):
    compat = _fork_compat_module()
    try:
...
# Keep upstream v0.21.4 implementations authoritative for names that still exist
# in this facade. Fork-only symbols are resolved lazily by ``__getattr__`` below;
# this avoids replacing newer task/dispatch semantics with the pre-upgrade
# monolith merely because a legacy plugin imports the old module path.
_FORK_COMPAT_OVERRIDE_NAMES = frozenset()
