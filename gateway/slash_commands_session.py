...
class GatewaySessionCommandsMixin:
    ...
    async def _handle_mycompress_command(self, event: MessageEvent) -> str:
        """Run the skill-guided compression wrapper through native compression."""
        try:
            from agent.mycompress import load_mycompress_focus, parse_mycompress_args
            keep_last, user_focus = parse_mycompress_args(event.get_command_args())
            focus_topic = load_mycompress_focus(user_focus, runtime_note="gateway")
        except (LookupError, ValueError) as exc:
            return f"(._.) /mycompress failed: {exc}"
        return await self._handle_compress_command(
            event, keep_last_override=keep_last, focus_topic_override=focus_topic
        )
    ...
    async def _handle_compress_command(
        ...
        event: MessageEvent,
        *,
        keep_last_override: int | None = None,
        focus_topic_override: str | None = None,
    ) -> str:
        ...
        if not getattr(getattr(self, "config", None), "multiplex_profiles", False):
            return await self._handle_compress_command_inner(
                event, keep_last_override=keep_last_override, focus_topic_override=focus_topic_override
            )
    ...
    async def _handle_compress_command_inner(
        self,
        event: MessageEvent,
        *,
        keep_last_override: int | None = None,
        ...
        if keep_last_override is not None:
            request = dataclasses.replace(request, partial=True, keep_last=keep_last_override)
        if focus_topic_override is not None:
            request = dataclasses.replace(request, focus_topic=focus_topic_override)
