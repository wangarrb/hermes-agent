...
class ShellFileOperations(LintMixin, SearchMixin, FileOperations):
    ...
    def _is_likely_binary(self, path: str, content_sample: str = None) -> bool:
        ...
        if content_sample:
            ...
            # them). A single replacement char at the sample boundary can instead
            # be a UTF-8 character cut by ``head -c``; ignore only that trailing
            # sentinel and keep interior U+FFFD read-only.
