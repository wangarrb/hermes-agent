...
VALUE_SIGNAL_PATTERNS: dict[str, tuple[int, list[str]]] = {
    ...
    "decision_or_conclusion": (
        4,
        [
            r"结论", r"决定", r"方案", r"推荐", r"不建议", r"取舍", r"原因", r"根因", r"风险",
            r"最终采用", r"终版", r"定版", r"确认使用", r"最终方案", r"选定",
        ],
    ),
    ...
    "pitfall_or_lesson": (
        5,
        [
            r"踩坑", r"坑", r"教训[：:\s]", r"注意[：:\s](?!.*心跳)(?!.*heartbeat)",
            r"避坑", r"不要用", r"不推荐", r"别用", r"不靠谱", r"有坑",
        ],
    ),
    "cross_day_signal": (
        3,
        [
            r"又出现", r"再次出现", r"反复出现", r"持续出现", r"一直有",
            r"和昨天一样", r"前几次", r"之前也", r"又是这个", r"老问题",
            r"再次遇到", r"多次", r"频繁",
        ],
    ),
}
...
DURABLE_SIGNAL_PATTERNS = [
    ...
    r"踩坑", r"坑", r"教训[：:\s]", r"避坑", r"不要用", r"不推荐", r"别用", r"不靠谱", r"有坑",
