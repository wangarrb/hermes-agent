...
def ns(**kw):
    defaults = dict(
        ...
        pre_existing_queue_wait_timeout=None,
        pre_existing_queue_poll=None,
        ...
        manifest_scan_state=Path('/tmp/manifest-scan-state.json'),
        execute=False,
        confirm=None,
...
def test_daily_plan_defaults_to_incremental_session_and_no_write():
    ...
    assert '--include-kanban' in manifest['command']
    assert '--scan-state' in manifest['command']
    assert manifest['command'][manifest['command'].index('--scan-state') + 1] == '/tmp/manifest-scan-state.json'
    ...
    drain = next(s for s in plan['steps'] if s['name'] == 'queue_drain_before_daily_retain')
    assert 'wait-queue' in drain['command']
    assert '--bank' in drain['command']
    assert drain['command'][drain['command'].index('--bank') + 1] == 'hermes'
    assert '--timeout' in drain['command']
    assert drain['command'][drain['command'].index('--timeout') + 1] == '86400'
    assert names(plan).index('queue_drain_before_daily_retain') < names(plan).index('build_session_manifest')
...
def test_session_profile_mode_can_be_overridden_for_manifest_build():
    ...
    assert '--include-codex' in manifest['command']


def test_codex_session_scan_can_be_disabled_for_daily_manifest_build():
    mod = load_module()
    plan = mod.build_plan(ns(mode='daily', include_codex=False))
    manifest = next(s for s in plan['steps'] if s['name'] == 'build_session_manifest')
    assert '--no-include-codex' in manifest['command']
    assert '--include-codex' not in manifest['command']
