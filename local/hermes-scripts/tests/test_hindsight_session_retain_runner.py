...
def test_retain_runner_rehydrates_hermes_markdown_artifact_lean_manifest(tmp_path):
    manifest_mod = load_module('hindsight_session_manifest')
    runner = load_module('hindsight_session_retain_runner')
    artifact = tmp_path / 'hermes-output.md'
    artifact.write_text('# Hermes Output\n\nMarkdown artifact from Hermes daily source.', encoding='utf-8')
    prod = manifest_mod.record_from_hermes_markdown_artifact(
        artifact,
        bank_target='hermes',
        producer='hermes_write_file_tool',
        source_session_path=str(tmp_path / 'session_hermes.json'),
    ...
    assert item['document_id'].startswith('hermes-artifact::planner::')
    assert item['context'] == 'hermes_markdown_artifact'
    assert item['metadata']['source_kind'] == 'hermes_markdown_artifact'
    assert item['metadata']['producer'] == 'hermes_write_file_tool'
    assert 'Markdown artifact from Hermes daily source' in item['content']


def test_retain_runner_rehydrates_kanban_prompt_markdown_lean_manifest(tmp_path):
    manifest_mod = load_module('hindsight_session_manifest')
    runner = load_module('hindsight_session_retain_runner')
    prompt = tmp_path / 'repo' / '.codex-kanban' / 'egomotion4d' / 'planner' / 't_kanban.md'
    prompt.parent.mkdir(parents=True)
    prompt.write_text('# Kanban Prompt\n\nRetain this Kanban prompt markdown.', encoding='utf-8')
    prod = manifest_mod.record_from_kanban_prompt_markdown(
        prompt,
        bank_target='hermes',
    )[0]
    paths = manifest_mod.write_manifest([prod], tmp_path, include_content=False)
    ...
    assert item['document_id'].startswith('kanban-markdown::')
    assert item['context'] == 'kanban_prompt_markdown'
    assert item['metadata']['source_kind'] == 'kanban_prompt_markdown'
    assert 'Retain this Kanban prompt markdown' in item['content']


def test_retain_runner_rehydrates_kanban_comment_lean_manifest(tmp_path):
    manifest_mod = load_module('hindsight_session_manifest')
    runner = load_module('hindsight_session_retain_runner')
    import sqlite3
    import time
    ...
    con.execute('INSERT INTO tasks (id,title,body,assignee,status,created_at,result) VALUES (?,?,?,?,?,?,?)', ('t_rehydrate', 'Kanban comment retain', '', 'critic', 'done', now, 'ok'))
    con.execute('INSERT INTO task_comments (task_id,author,body,created_at) VALUES (?,?,?,?)', ('t_rehydrate', 'planner', 'Rehydrate this Kanban comment.', now + 1))
    con.commit()
    con.close()
    prod = manifest_mod.records_from_kanban_board_db(
        'egomotion4d',
        db,
        bank_target='hermes',
    ...
    assert item['document_id'] == 'kanban-comment::egomotion4d::t_rehydrate::1'
    assert item['context'] == 'kanban_comment'
    assert item['metadata']['source_kind'] == 'kanban_task_comment'
    assert 'Rehydrate this Kanban comment' in item['content']
...
def test_scan_state_updates_only_after_execute_success(tmp_path):
    manifest_mod = load_module('hindsight_session_manifest')
    runner = load_module('hindsight_session_retain_runner')
    session_file = tmp_path / 'session_scan_state.json'
    make_session(session_file, session_id='scan-state')
    prod = manifest_mod.records_from_json_file(session_file, bank_target='hermes_v3')[0]
    paths = manifest_mod.write_manifest([prod], tmp_path / 'manifest', include_content=False)
    ...
    dry = runner.run_manifest(paths['manifest'], client=FakeClient(), dry_run=True, scan_state_path=scan_state_path)
    assert dry['scan_state_updated'] is False
    assert not scan_state_path.exists()
        ...
        dry_run=False,
        confirm=runner.RETAIN_CONFIRM,
        submit_state_path=tmp_path / 'submit-state.json',
        scan_state_path=scan_state_path,
    )

    assert done['scan_state_updated'] is True
    saved = json.loads(scan_state_path.read_text(encoding='utf-8'))
    assert saved['sources']['hermes_json']['max_source_mtime_ns'] == prod['metadata']['source_mtime_ns']
