...
def main() -> None:
    ...
    # HINDSIGHT_OFFLINE_* env vars override argparse defaults (set in ~/.hermes/.env)
    _offline_env = lambda k, fallback: os.environ.get(k, '') or fallback
    parser.add_argument("--llm-model", default=_offline_env("HINDSIGHT_OFFLINE_LLM_MODEL", MINIMAX_MODEL), help="OpenAI-compatible chat/completions model name")
    parser.add_argument("--llm-base-url", default=_offline_env("HINDSIGHT_OFFLINE_LLM_BASE_URL", MINIMAX_BASE_URL), help="OpenAI-compatible base URL, e.g. https://api.minimaxi.com/v1")
    parser.add_argument("--llm-api-key-env", default=_offline_env("HINDSIGHT_OFFLINE_LLM_API_KEY_ENV", "MINIMAX_API_KEY"), help="API key env var name; value is read from env or ~/.hermes/.env")
