...
BOOTSTRAP_DIAGNOSTIC_RE = re.compile(
    r"(who are u|who are you|你是谁|你是.*助手|检查下你的环境|检查环境|hermes claw migrate|claw migrate|上下文上限|context.*上限|重启了.*上下文|lossless-claw|lcm\.db|model_metadata|glm-5.*上下文)",
    re.IGNORECASE,
)
# Self-identification ("我是臭臭" / assistant intro) matched the bootstrap
# diagnostic pattern above and caused whole sessions containing real work
# (e.g. the 2026-08-28 / 2026-09-04 MonoPixel paper analysis in session
# 20260826_161200_486f0e) to be routed to manual_review and silently never
# retained (submitted_items=0 from 2026-08-27 through 2026-09-04).
# Assistant self-introduction is triggered by ordinary model-switch notes in
# normal conversations, so it must NOT classify an entire session as a
# bootstrap/diagnostic session.
...
def session_event_date(session_data: dict[str, Any]) -> str | None:
    ...
    Prefer the latest per-message timestamp when the session snapshot carries
    them (newer Hermes builds stamp ``timestamp`` on each message at persist
    time). Resumed sessions are REUSED with their original filename and
    session_start, so a session whose conversation spans several days must not
    be dated by its (possibly weeks-old) start time — that misdated every
    fact extracted from the resumed portion. Fall back to session-level
    timestamps for older snapshots without per-message times. This must be
    ...
    if msg_ts:
        return max(msg_ts)
    for key in ["last_updated", "ended_at", "session_start", "started_at", "created_at"]:
