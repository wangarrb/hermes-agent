...
def native_env_snapshot() -> tuple[bool, dict[str, str] | str]:
    """读取 systemd EnvironmentFile（native pip 部署，hindsight.service）。"""
    if not HINDSIGHT_NATIVE_ENV_FILE.is_file():
        return False, f"{HINDSIGHT_NATIVE_ENV_FILE} not found"
    env: dict[str, str] = {}
    ...
    if not env:
        return False, f"{HINDSIGHT_NATIVE_ENV_FILE} empty"
    return True, env
...
def runtime_env_snapshot(container: str = "hindsight") -> tuple[bool, dict[str, str] | str]:
    """读取 Hindsight 运行时环境变量。

    native pip 部署优先读 systemd EnvironmentFile（hindsight-native.env）；
    docker 部署兜底用 docker inspect（容器停止时仍返回其持久化配置，仅迁移期参考）。
    """
    ok, data = native_env_snapshot()
    if ok:
        return True, data
    return docker_env_snapshot(container)
...
def run_preflight(cfg: dict[str, Any], *, strict_runtime: bool = False) -> dict[str, Any]:
    ...
    if docker_path:
        ...
        docker_running = proc.returncode == 0 and proc.stdout.strip().lower() == "true"
    # native pip 部署：容器可停止，只要 systemd 服务 active 即视为运行中
    systemctl_path = shutil.which("systemctl")
    if systemctl_path:
        sp = run([systemctl_path, "is-active", "hindsight.service"], timeout=10)
        native_active = sp.returncode == 0 and sp.stdout.strip() == "active"
    running = docker_running or native_active
    checks.append(check("hindsight_container_running", running, severity="block" if docker_required else "warn", detail={"container": container, "docker_running": docker_running, "systemd_active": native_active, "native_env_file": str(HINDSIGHT_NATIVE_ENV_FILE)}))
    if docker_path or native_active:
        env_ok, env_data = runtime_env_snapshot(container)
    ...
    # Runtime (container-side) tuning —— native systemd 部署下容器不运行，跳过容器文件检查
    # （运行时 tuning 已由上方 v061_runtime_env_official_tuning 基于 native env 校验）
    ...
    if docker_running and tuning_container_path:
