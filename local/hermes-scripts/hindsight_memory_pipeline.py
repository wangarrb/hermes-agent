...
def session_manifest_step(args: argparse.Namespace) -> Step:
    ...
    return Step(
        ...
        description="Build cleaned session/json manifest from default Hermes plus Hindsight-backed profiles (auto-discovered under ~/.hermes/profiles). Local files only; no Hindsight writes.",
        command=cmd,
        mutating=False,
        writes_local=True,
...
def retain_session_step(args: argparse.Namespace) -> Step:
    cmd = [
        ...
        *common_wrapper_args(args),
        "--enable-observations",
        "--no-switch-mode" if getattr(args, "no_switch_mode", True) else "",
        "--manifest",
        "{{manifest}}",
        "--bank",
        args.bank,
...
def manual_review_sample_step(args: argparse.Namespace) -> Step:
    """Score + sample manual_review backlog; LLM promotes real-work records.

    When --submit is passed the sampler itself submits promoted records through
    hindsight_session_retain_runner (same confirm token as the main retain),
    so no extra placeholder plumbing is needed in execute_plan.
    """
    cmd = [
        sys.executable,
        py("hindsight_manual_review_sampler.py"),
        "--manifest",
        "{{manifest}}",
        "--sample-size",
        str(int(getattr(args, "manual_review_sample_size", 5) or 5)),
        "--output-dir",
        str(path_from_config(get_cfg(args), "manifest_dir")),
        "--bank",
        args.bank,
    ...
    return Step(
        name="manual_review_sampling",
        description="Importance-score the manual_review backlog, sample top-N, LLM-judge each, and retain promoted records (writes Hindsight only for promoted docs).",
        command=cmd,
        mutating=bool(args.execute),
        writes_local=True,
        consumes={"manifest": "build_session_manifest"},
...
def daily_reflect_step(args: argparse.Namespace) -> Step:
    ...
    no_switch_arg = "--no-switch-mode" if getattr(args, "no_switch_mode", True) else ""
    if args.execute:
        cmd = [
            sys.executable,
            py("hindsight_minimax_import.py"),
...
def build_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.mode == "daily" or (args.mode == "full" and not getattr(args, "skip_daily", False)):
        steps.extend([
            session_manifest_step(args),
            retain_session_step(args),
            manual_review_sample_step(args),
            daily_reflect_step(args),
        ])
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--no-switch-mode", action="store_true", default=True, help="Skip LLM provider switch_mode in retain/reflect wrappers; container is pre-configured via start-hindsight.sh (default: True)")
