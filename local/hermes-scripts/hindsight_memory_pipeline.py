...
def session_manifest_step(args: argparse.Namespace) -> Step:
    scan_state = getattr(args, "manifest_scan_state", None)
    if scan_state is None:
        scan_state = HERMES_HOME / "hindsight" / "session_ingest" / "manifest_scan_state.json"
    cmd = [
        sys.executable,
    ...
    if getattr(args, "include_deepseek", True):
        cmd.append("--include-deepseek")
    else:
        cmd.append("--no-include-deepseek")
    if getattr(args, "include_kanban", True):
        cmd.append("--include-kanban")
    else:
        cmd.append("--no-include-kanban")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--manifest-scan-state", type=Path, default=None, help="Per-source manifest candidate scan state. Default: ~/.hermes/hindsight/session_ingest/manifest_scan_state.json")
    ...
    ap.add_argument("--include-deepseek", dest="include_deepseek", action="store_true", default=os.environ.get("HINDSIGHT_INCLUDE_DEEPSEEK", "1").lower() not in {"0", "false", "no", "off"}, help="Include DeepSeek-TUI session JSON files in the daily manifest build (default on)")
