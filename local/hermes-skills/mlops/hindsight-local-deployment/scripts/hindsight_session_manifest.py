...
def record_from_codex_markdown_artifact(
    ...
    *,
    ...
    tags = sorted(set(["source:codex-artifact", "topic:markdown-artifact", *propose_tags(full_text, {})]))
    action, reason = ("manual_review", "secret_or_credential_material") if contains_secret_material(full_text) else ("production", "codex_markdown_artifact")
    source_meta = source_file_metadata(path, raw_bytes)
    chunks = split_text(full_text, DEFAULT_MAX_DOCUMENT_CHARS)
    records: list[dict[str, Any]] = []
    base_id = codex_artifact_document_id(path)
    for idx, chunk in enumerate(chunks or [full_text]):
        document_id = base_id if len(chunks) <= 1 else f"{base_id}::part-{idx:03d}"
        content = clean_text(
            "\n".join([
                f"Title: {path.stem}",
                f"Source: Codex generated Markdown artifact",
                f"Path: {path}",
                "",
                chunk,
            ])
        ...
        metadata = {
            "source_kind": "codex_markdown_artifact",
            "source_label": "codex",
        ...
        records.append({
            ...
            "observation_scopes": observation_scopes_for_tags(tags),
            "metadata": metadata,
            "context": "codex_markdown_artifact",
            "update_mode": "replace",
        })
    return records
