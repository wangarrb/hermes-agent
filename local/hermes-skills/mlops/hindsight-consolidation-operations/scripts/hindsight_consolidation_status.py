...
def operation_public_view(op: dict[str, Any]) -> dict[str, Any]:
    ...
    Extracts llm_calls from result_metadata when present.
    """
    rm = op.get("result_metadata") or {}
    llm_calls = rm.get("llm_calls") if isinstance(rm, dict) else None
    return {
...
def fetch_operations_api_summary(
    *,
    ...
    # Fetch llm_calls from result_metadata for recent completed consolidation operations
    llm_calls_total: int | None = None
    try:
        completed_data = api_get_json(
            bank_url(api_url, tenant, bank, "operations"),
            params={"status": "completed", "limit": 50, "offset": 0, "exclude_parents": exclude_parents},
            timeout=timeout,
        )
        consolidation_ops = [
            op for op in (completed_data.get("operations") or [])
            if isinstance(op, dict) and (op.get("task_type") or op.get("operation_type")) == "consolidation"
        ]
        if consolidation_ops:
            ...
            for op in consolidation_ops:
                ...
                try:
                    detail = api_get_json(
                        f"{bank_url(api_url, tenant, bank, f'operations/{op_id}')}",
                        timeout=timeout,
                    )
                    rm = detail.get("result_metadata") or {}
                    if isinstance(rm, dict) and "llm_calls" in rm:
                        llm_calls_total += rm["llm_calls"]
                except Exception:
                    pass  # Skip individual fetch failures
    except Exception:
        pass  # llm_calls aggregation is best-effort
...
def build_report(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.skip_operations_api:
        ...
        try:
            ...
            # Add llm_calls to bank_summary from operations API
            llm_calls_total = api_ops.get("llm_calls_total")
            if llm_calls_total is not None:
                report["bank_summary"]["llm_calls"] = llm_calls_total
        except Exception as exc:
...
def render_human_summary(report: dict[str, Any]) -> str:
    ...
    # Show llm_calls from operations API if available
    llm_calls = ((ops_api.get("data") or {}).get("llm_calls_total") if ops_api.get("ok") else None)
    if llm_calls is not None:
        lines.append(f"llm_calls (recent consolidations): {llm_calls}")
    elif llm_calls is None and ops_api.get("ok"):
        lines.append("llm_calls: unknown (no consolidation ops in sample)")
