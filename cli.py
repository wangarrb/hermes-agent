...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        _cli_visible_print()
        _cli_visible_print(f"  {'#':<3} {'Title':<24} {'Msgs':<6} {'CTX':<8} {'Last Active':<13} {'ID'}")
        _cli_visible_print(f"  {'─' * 3} {'─' * 24} {'─' * 6} {'─' * 8} {'─' * 13} {'─' * 24}")
        for idx, session in enumerate(sessions, start=1):
            title = (session.get("title") or "—")[:22]
            # Fork customization: show message count and context token estimate
            msg_count = session.get("message_count", "—")
            ctx_tokens = session.get("context_tokens", "")
            ctx_display = f"{ctx_tokens // 1000}K" if isinstance(ctx_tokens, int) and ctx_tokens else "—"
            last_active = _relative_time(session.get("last_active"))
            _cli_visible_print(f"  {idx:<3} {title:<24} {msg_count:<6} {ctx_display:<8} {last_active:<13} {session['id']}")
        _cli_visible_print()
    ...
    def _handle_mycompress_command(self, cmd_original: str = ""):
        ...
        """
        import re as _re
        _raw_args = (cmd_original.split(None, 1)[1] if " " in cmd_original else "").strip()
        ...
        try:
            ...
            pass  # skill loading failure is non-critical

        # Build the /compress command and delegate
        if keep_rounds is not None and keep_rounds > 0:
            compress_cmd = f"/compress here {keep_rounds * 2}"
            if focus_topic:
