...
class HermesCLI:
    ...
    def _handle_mycompress_command(self, cmd_original: str = ""):
        ...
        then delegates to _manual_compress with modified command text.
        """
        # Parse arguments
        ...
        try:
            args, remaining = parser.parse_known_args(shlex.split(raw_args))
        except Exception:
            args = argparse.Namespace(keep_last_rounds=None)
            remaining = []

        user_focus = " ".join(remaining).strip() if remaining else ""
        keep_last_rounds = args.keep_last_rounds

        # Load mycompress skill content as focus topic
        from pathlib import Path as _P
        skill_dir = _P.home() / ".hermes" / "skills" / "note-taking" / "mycompress"
        ...
        # Build compress command text
        # v0.16.0 /compress supports "here [N]" and "<focus>" forms
        if keep_last_rounds and keep_last_rounds > 0:
            compress_cmd = f"/compress here {keep_last_rounds}"
            if focus_topic:
                compress_cmd += f" {focus_topic}"
        elif focus_topic:
            compress_cmd = f"/compress {focus_topic}"
        else:
            compress_cmd = "/compress"

        # Delegate to _manual_compress
